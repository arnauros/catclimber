...
function fetchRoadData(location, radius = 5000) {
  const url = `https://api.mapbox.com/v4/mapbox.mapbox-terrain-v2/tilequery/${location[0]},${location[1]}.json?radius=${radius}&limit=50&access_token=${mapboxToken}`;
...
function visualizeRoads(features) {
  features.forEach((feature, index) => {
    ...
    console.log(`Added layer for road ${index}`);
  });
}
